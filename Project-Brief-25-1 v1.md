...
| (4) 지도교수 | 심재형 교수 |
| (5) 트랙  | 산학 |
| (6) 과제 키워드 | ChatGPT-4.1 api, 개인 맞춤형 코디 추천, 패션 추천에 특화된 기능형 챗봇, 창의적 패션 조합|
| (7) 과제 내용 요약 | AD*RESS는 외출복 선택에 어려움을 겪는 MZ세대 여성을 위한 개인화된 패션 조합 추천 서비스이다. 사용자로부터 입력받은 요구사항과 외부 API(캘린더, 날씨, 위치)를 통해 수집된 데이터를 GPT-4.1로 분석하여, 현재 맥락을 반영한 키워드 및 벡터값을 도출한다. 이를 기반으로 TPO와 날씨, 사용자의 추가적인 니즈에 부합하는 의류를 디지털 옷장(사용자 등록 의류) 또는 쇼핑몰 기반 데이터셋(사전 수집 의류)에서 검색하고, 추천 알고리즘을 통해 적절한 코디 조합을 제안한다. |
